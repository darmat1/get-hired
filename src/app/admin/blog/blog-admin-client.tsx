...
export default function BlogAdminClient({
  ...
  useEffect(() => {
    checkBlogPostsJsonExists().then(setHasPostsJson);
  }, []);
  ...
  const handleFillFromJson = async () => {
    setIsFilling(true);
    try {
      const result = await getNextPendingPost();
      if ("error" in result) {
        alert(result.error);
        return;
      }
      ...
      if (!result.nextPost) {
        alert("No more pending posts in posts.json!");
        return;
      }

      const { slug, topic, prompt } = result.nextPost;
      setFormData({
        ...formData,
        slug,
        topic,
      ...
      console.error("[Blog] Fill from JSON error:", err);
      alert("Error filling from JSON");
    } finally {
      setIsFilling(false);
    }
  };
      ...
      <div className="flex-1 w-full max-w-7xl bg-white dark:bg-slate-900 p-4 sm:p-6 lg:p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            ...
            {!editingPostId && hasPostsJson && (
              <button
                type="button"
                onClick={handleFillFromJson}
                disabled={isFilling}
                className="px-4 py-2 text-sm font-medium text-emerald-600 dark:text-emerald-400 hover:text-emerald-900 dark:hover:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg transition flex items-center justify-center gap-2 border border-emerald-100 dark:border-emerald-800/50 shadow-sm"
              >
                {isFilling ? (
