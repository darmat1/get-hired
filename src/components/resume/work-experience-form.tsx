...
export function WorkExperienceForm({ data, onChange, onNext, onBack }: WorkExperienceFormProps) {
  ...
  const removeDescriptionPoint = (expIndex: number, descIndex: number) => {
    const updated = [...data]
    const descriptions = updated[expIndex].description.filter((_, i) => i !== descIndex)
    updated[expIndex] = { ...updated[expIndex], description: descriptions }
    onChange(updated)
  }
  ...
  return (
    <div className="bg-card text-card-foreground rounded-lg border border-border shadow-sm p-6">
      ...
      <div className="space-y-6">
        {data.map((exp, index) => (
          <div key={exp.id} className="border border-border rounded-lg p-4 bg-background/50">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-lg font-medium">{t('form.work_experience')} #{index + 1}</h3>
              <Button
              ...
              <div className="md:col-span-2">
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none w-fit">
                    ...
                    />
                    <span className="text-sm text-muted-foreground">{t('work.current_position')}</span>
                  </label>
                </div>
              </div>
            </div>
