...
function SectionHeader({
  ...
  return (
    <div
      className="flex justify-between items-end pt-1.5 mb-2.5"
...
export function ModularPreview({ data, onChange, isEditing }: Props) {
  ...
  const updateCustomization = (key: string, value: any) => {
    if (!onChange) return;
    onChange({ ...data, customization: { ...customization, [key]: value } });
  };
  ...
  return (
    <div
      className="font-sans space-y-5 p-7 bg-white h-full min-h-[1056px] relative group/preview"
    ...
    >
      ...
      {/* Header */}
      <div className="relative flex items-center gap-4 pb-1">
        {isEditing && (
          <button
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[20px] font-bold text-slate-900 leading-tight flex flex-wrap gap-1.5">
            <EditableText
          ...
          {(data as any).targetPosition && (
            <div className="text-[11px] mt-0.5" style={{ color: accentColor }}>
              {(data as any).targetPosition}
            </div>
          )}
      ...
      <div>
        <SectionHeader
          title={getTranslation("form.summary", data.language || "en")}
          accentColor={accentColor}
        />
        <EditableText
          value={personalInfo.summary || ""}
          onChange={(v) => updatePersonalInfo("summary", v)}
          className="text-slate-700 text-[10.5px] leading-relaxed text-justify block"
          multiline
                          ...
                          <div className="flex justify-between items-start mb-0.5">
                            <div className="flex-1 pr-4">
                              <EditableText
                                ...
                                placeholder="Title"
                              />
                              <div className="text-slate-500 text-[10px] flex gap-1">
                                <EditableText
                                  value={exp.company || ""}
                                  onChange={(v) => {
                                    const newExp = [...(workExperience || [])];
                                  ...
                                  <>
                                    ...
                                    {isEditing ? (
                                      <select
                                        ...
                                        className="bg-transparent border-none p-0 m-0 outline-none text-[10px] text-slate-500 hover:text-slate-700 transition-colors cursor-pointer appearance-none"
                                      >
                                        <option value="">
                                          {getTranslation(
                            ...
                            <div className="shrink-0 text-right text-[10px] text-slate-500 w-[140px]">
      ...
      <div>
        <SectionHeader
          title={getTranslation("form.education", data.language || "en")}
          accentColor={accentColor}
        >
          {isEditing && (
            <button
            ...
            <div key={edu.id} className="relative group/edu">
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-4">
                  ...
                  />
                  <div className="text-slate-500 text-[10px] flex gap-1 flex-wrap">
                    <EditableText
                      value={edu.degree || ""}
                      onChange={(v) => {
                        const newEdu = [...(education || [])];
      ...
      {(isEditing || technicalAndSoftSkills.length > 0) && (
        <div>
          <SectionHeader
            title={getTranslation("form.skills", data.language || "en")}
            accentColor={accentColor}
          >
            {isEditing && (
            ...
            {technicalAndSoftSkills.map((skill) => (
              <div key={skill.id} className="relative group/skill">
                <EditableText
                  ...
                  className="text-[10px] bg-slate-100 px-2 py-1 rounded text-slate-800"
                />
                {isEditing && (
                  <button
      ...
      {(isEditing || languageSkills.length > 0) && (
        <div>
          <SectionHeader
            title={getTranslation("skills.languages", data.language || "en")}
            accentColor={accentColor}
          >
            {isEditing && (
              <div className="flex gap-1">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1.5">
            {languageSkills.map((skill) => (
              <div key={skill.id} className="relative group/skill flex items-center gap-1">
                <EditableText
                  value={skill.name || ""}
                  onChange={(v) => {
                    const newSkills = [...(skills || [])];
                ...
                {isEditing && (
                  <button
                    onClick={() => removeSkill(skill.id)}
                    className="opacity-0 group-hover/skill:opacity-100 text-red-400 hover:text-red-600 transition-all"
                  >
                    <Trash2 size={10} />
                  </button>
                )}
      ...
      {(isEditing || (certificates || []).length > 0) && (
        <div>
          <SectionHeader
            title={getTranslation("form.certificates", data.language || "en")}
            accentColor={accentColor}
          >
            {isEditing && (
              <button
          ...
          <div className="space-y-2">
            {(certificates || []).map((cert, idx) => (
              <div key={cert.id} className="relative group/cert flex justify-between items-start gap-2">
                {isEditing && (
                  <button
                ...
                <EditableText
                  ...
                  className="text-[10px] text-slate-500 shrink-0 text-right"
                  placeholder="Date"
                />
              </div>
            ))}
