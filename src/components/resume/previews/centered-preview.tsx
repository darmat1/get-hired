...
export function CenteredPreview({ data, onChange, isEditing }: Props) {
  ...
  return (
    <div className="font-sans bg-white h-full min-h-[1056px] relative group/preview overflow-hidden">
      {/* Banner Header */}
      <div
        className="relative flex flex-col items-center gap-1 p-7 text-white transition-colors duration-300"
      ...
      >
        ...
        {isEditing && isSettingsOpen && (
          <div
            ref={settingsRef}
            className="absolute top-14 right-4 bg-slate-900 shadow-2xl rounded-xl border border-white/10 p-4 z-50 animate-in fade-in zoom-in duration-300 w-64 text-left"
          >
        ...
        {showAvatar &&
          (personalInfo.avatarUrl ? (
            <img
              ...
              className="w-[76px] h-[76px] rounded-full object-cover border-2 border-white/50 mb-2"
            />
          ) : (
            <div className="w-[76px] h-[76px] rounded-full bg-white/15 flex items-center justify-center text-xl font-bold text-white/60 border-2 border-white/30 mb-2">
              {personalInfo.firstName?.[0]}
              {personalInfo.lastName?.[0]}
            </div>
          ))}

        <div className="text-[20px] font-bold uppercase leading-tight tracking-wide flex flex-wrap gap-2 justify-center">
          <EditableText
        ...
        {headerContacts.length > 0 && (
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-white/90 text-[11px] mt-1">
      ...
      <div className="flex">
        {/* Sidebar */}
        <div className="w-[30%] shrink-0 p-5 bg-gray-50">
          {sidebarLinks.length > 0 && (
            <div className="mb-6">
              <h2
                className="text-[11px] font-bold uppercase mb-2 border-b pb-0.5"
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1 pr-4">
                        <EditableText
                          value={cert.name || ""}
                          onChange={(v) =>
                            updateCertificate(cert.id, "name", v)
                          }
                          className="font-bold text-slate-900 text-[11px]"
                        ...
                        {(isEditing || cert.url) && (
                          <EditableText
                            value={cert.url || ""}
                            onChange={(v) =>
                              updateCertificate(cert.id, "url", v)
                            }
                            placeholder="URL"
                      ...
                      <div className="shrink-0 text-right text-[10px] text-slate-600 w-[80px]">
                        <EditableText
                          value={cert.date || ""}
                          onChange={(v) =>
                            updateCertificate(cert.id, "date", v)
                          }
                          placeholder="Date"
                          className="text-right w-full"
                        />
