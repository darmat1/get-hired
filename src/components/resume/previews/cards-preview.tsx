...
// Card wrapper: bordered, rounded, light fill, with a title "tab" that
// overlaps the top border (negative margin pulls the card up under the tab).
function Card({
  title,
  accentColor,
  actions,
  ...
  return (
    <div className="mb-5">
      <div className="flex items-center justify-between">
        <span
          className="relative z-10 inline-block ml-3 -mb-[9px] px-2 bg-white text-[10px] font-bold uppercase border border-slate-300 rounded"
          style={{ color: accentColor }}
        >
          {title}
        </span>
        {actions && (
          <div className="relative z-10 -mb-[9px] mr-1 flex gap-1 bg-white px-1 rounded">
            {actions}
          </div>
        )}
      </div>
      <div
        className="rounded-lg border border-slate-300 pt-5 pb-4 px-4"
...
export function CardsPreview({ data, onChange, isEditing }: Props) {
  ...
  const showSkillsCard = isEditing || technicalAndSoft.length > 0;
  const showLanguagesCard = isEditing || languages.length > 0;

  return (
    <div
      className="font-sans p-6 bg-white h-full min-h-[1056px] relative group/preview"
    ...
    >
      ...
      {/* Header */}
      <div className="relative flex items-center gap-4 mb-6 pb-2">
        {isEditing && (
          <button
        ...
        <div>
          <div
            className="text-[20px] font-bold uppercase tracking-wide leading-none"
            style={{ color: "#111827" }}
          >
          ...
          {(data as any).targetPosition && (
            <div
              className="text-[11px] mt-0.5 mb-1"
              style={{ color: accentColor }}
            >
              {(data as any).targetPosition}
            </div>
      ...
      {/* Summary */}
      <Card title={getTranslation("form.summary", lang)} accentColor={accentColor}>
        <EditableText
      ...
      {workExperience && (
        <Card
          title={getTranslation("form.work_experience", lang)}
            ...
            isEditing && (
              <>
                <button
                  ...
                  className="text-[10px] text-slate-500 hover:text-slate-800 flex items-center gap-1 px-1.5 py-0.5 rounded-md transition-all hover:bg-slate-100"
                >
                  <Import size={12} /> {t("profile.btn_import")}
                </button>
                <button
                  onClick={() =>
                    updateSection("workExperience", [
      ...
      {/* Education */}
      <Card
        title={getTranslation("form.education", lang)}
        accentColor={accentColor}
        actions={
          isEditing && (
            <button
      ...
      {/* Skills + Languages */}
      {(showSkillsCard || showLanguagesCard) && (
        <div className="flex gap-4">
          {showSkillsCard && (
            <div className="flex-1">
              <Card
                title={getTranslation("form.skills", lang)}
                accentColor={accentColor}
                actions={
                  isEditing && (
                    <>
                      <button
                        ...
                        className="text-[9px] text-slate-500 hover:text-slate-800 flex items-center gap-1 px-1 py-0.5 rounded transition-colors hover:bg-slate-100"
                      >
                        <Import size={10} />
                      </button>
                      <button
          ...
          {showLanguagesCard && (
            <div className="flex-1">
              <Card
                title={getTranslation("skills.languages", lang)}
                accentColor={accentColor}
                actions={
                  isEditing && (
                    <>
      ...
      {/* Certificates */}
      <Card
        title={getTranslation("form.certificates", lang)}
        accentColor={accentColor}
        actions={
          isEditing && (
            <button
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-4">
                  ...
                  <EditableText
                    ...
                    className="text-slate-700 text-[10px] italic"
