...
export function BannerPreview({ data, onChange, isEditing }: Props) {
  ...
  const tintColor = hexToRgba(accentColor, 0.08);
  ...
  return (
    <div className="font-sans bg-white h-full min-h-[1056px] relative group/preview overflow-hidden">
      {/* Banner Header */}
      <div
        className="relative flex items-center gap-5 p-7 text-white transition-colors duration-300"
      ...
      >
        ...
        {showAvatar &&
          (personalInfo.avatarUrl ? (
            <img
              ...
              className="w-[86px] h-[86px] rounded-xl object-cover shrink-0"
            />
          ) : (
            <div className="w-[86px] h-[86px] rounded-xl bg-white/15 flex items-center justify-center text-xl font-bold text-white/60 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[22px] font-bold uppercase leading-tight tracking-wide flex flex-wrap gap-2">
            <EditableText
      ...
      <div className="flex">
        {/* Sidebar */}
        <div
          className="w-[30%] shrink-0 p-5"
          style={{ backgroundColor: tintColor }}
        >
          {(combinedSkills.length > 0 || isEditing) && (
            <div className="mb-6">
              <div className="flex justify-between items-end mb-2 border-b pb-0.5" style={{ borderColor: accentColor }}>
                <h2
          ...
          <div>
            ...
            <div className="space-y-3">
              {(education || []).map((edu, idx) => (
                <div
                  key={edu.id}
                  className="relative group/edu pl-3"
                  style={{ borderLeft: `3px solid ${accentColor}` }}
                >
                  {isEditing && (
                    <button
