...
export function LayeredPreview({ data, onChange, isEditing }: Props) {
  ...
  const lightTint = `rgba(${r}, ${g}, ${b}, 0.1)`;
  const darkTint = `rgb(${Math.round(r * 0.55)}, ${Math.round(g * 0.55)}, ${Math.round(b * 0.55)})`;
  ...
  return (
    <div
      className="font-sans bg-white h-full min-h-[1056px] relative group/preview"
    ...
    >
      ...
      {/* Tier 1: Light tint - avatar, name, target position, links */}
      <div
      ...
      >
        {isEditing && (
          <button
            ...
            className="absolute top-2 right-2 text-[10px] text-slate-500 hover:text-slate-800 flex items-center gap-1.5 px-2 py-1 rounded-md transition-all hover:bg-white/50"
        ...
        {showAvatar && personalInfo.avatarUrl && (
          <img
            ...
            className="w-[70px] h-[70px] rounded-lg object-cover shrink-0"
          />
        )}
      ...
      {/* Tier 2: Dark tint - contact band */}
      {contactItems.length > 0 && (
        <div
          className="flex flex-wrap justify-center items-center gap-x-4 gap-y-1 px-6 py-2"
          style={{ backgroundColor: darkTint }}
        >
      ...
      {/* Body: main (left 70%) + sidebar (right 30%) */}
      <div className="flex pt-4">
        {/* Main */}
        <div className="w-[70%] px-6 space-y-4">
          {/* Summary */}
          <div>
            <h2
              className="text-[12px] font-bold mb-1.5 uppercase"
              style={{ color: accentColor }}
            >
        ...
        {/* Sidebar (right) */}
        <div className="w-[30%] px-4 pr-6 space-y-4">
                  ...
                  {categorySkills.map((skill) => (
                    <div key={skill.id} className="relative group/skill">
                      <EditableText
                        ...
                        style={{ backgroundColor: lightTint, color: "#1f2937" }}
                      />
                      {isEditing && (
                        <button
                          onClick={() =>
              ...
              <div className="flex justify-between items-end mb-2">
                ...
                {isEditing && (
                  <button
                    ...
                    className="text-[9px] text-slate-500 hover:text-slate-800 flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors hover:bg-slate-100 ml-2"
                  >
                    <Plus size={10} /> Add
                  </button>
                )}
                  ...
                  <div key={cert.id} className="relative group/cert">
                    {isEditing && (
                      <button
                        ...
                        className="absolute -right-1 -top-1 opacity-0 group-hover/cert:opacity-100 text-slate-300 hover:text-red-500 transition-opacity p-0.5"
                    ...
                    <EditableText
                      ...
                      className="font-bold text-slate-900 text-[9.5px] block"
                      placeholder="Certificate Name"
                    />
                    <div className="text-slate-600 text-[8.5px] flex gap-1 flex-wrap">
                      <EditableText
                        value={cert.issuer || ""}
                        onChange={(v) => {
                          const newCerts = [...(certificates || [])];
