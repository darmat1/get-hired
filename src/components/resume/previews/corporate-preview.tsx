...
export function CorporatePreview({ data, onChange, isEditing }: Props) {
  ...
  const accentColor = customization?.sidebarColor || "#374151";
  ...
  const contactItems = [
    { key: "email", show: showEmail, value: personalInfo.email || "", placeholder: "Email" },
    { key: "phone", show: showPhone, value: personalInfo.phone || "", placeholder: "Phone" },
    { key: "location", show: showAddress, value: personalInfo.location || "", placeholder: "Location" },
    { key: "telegram", show: showTelegram, value: personalInfo.telegram || "", placeholder: "Telegram" },
  ].filter((item) => item.show);

  const profileLinks = [
    { key: "linkedin", show: showLinkedin, value: personalInfo.linkedin || "", placeholder: "LinkedIn" },
    { key: "github", show: showGithub, value: personalInfo.github || "", placeholder: "GitHub" },
    { key: "website", show: showWebsite, value: personalInfo.website || "", placeholder: "Website" },
  ].filter((item) => item.show);

  const mainSectionTitle = "text-[12px] font-bold mb-1.5 uppercase border-b pb-0.5";
  const sidebarSectionTitle = "text-[10px] font-bold mb-1.5 uppercase border-b pb-0.5 text-slate-900";

  return (
    <div
      className="flex h-full min-h-[1056px] w-full bg-white font-sans relative group/preview"
    ...
    >
      ...
      {/* SIDEBAR (Left - 30%, plain neutral) */}
      <div className="w-[30%] bg-slate-50 p-5 flex flex-col shrink-0">
        {isEditing && (
          <button
            onClick={importPersonalInfo}
            disabled={isImportingPersonalInfo}
            className="self-center text-[9px] text-slate-500 hover:text-slate-800 flex items-center gap-1.5 px-2 py-1 mb-2 rounded-md transition-all hover:bg-slate-200"
            title="Import from Profile"
          >
            {isImportingPersonalInfo ? (
              <div className="w-2.5 h-2.5 border border-t-slate-500 border-slate-200 rounded-full animate-spin" />
            ) : (
              <Import size={10} />
            )}
            Import
        ...
        <div className="text-center mb-4">
          <div className="text-[15px] font-bold text-slate-900 leading-tight">
            <div className="inline-flex flex-nowrap justify-center items-center gap-1.5 whitespace-nowrap">
              <EditableText
        ...
        {(contactItems.length > 0 || profileLinks.length > 0) && (
          <div className="mb-4">
            <h3 className={sidebarSectionTitle} style={{ borderColor: accentColor }}>
            ...
            <div className="space-y-1.5">
              {contactItems.map((item) => (
                <EditableText
                  ...
                  className="text-[9px] text-slate-700 block break-words"
                />
              ))}
              {profileLinks.map((item) => (
                <EditableText
                  ...
                  placeholder={item.placeholder}
                  className="text-[9px] block break-words"
          ...
          return (
            <div key={cat} className="mb-4">
              <div className="flex justify-between items-center">
                <h3 className={sidebarSectionTitle + " flex-1"} style={{ borderColor: accentColor }}>
                  {cat === "technical"
              ...
              {isEditing && (
                <div className="flex gap-1 mb-1.5">
                  <button
                    ...
                    className="text-[9px] text-slate-500 hover:text-slate-800 flex items-center gap-1 px-1.5 py-0.5 rounded transition-colors hover:bg-slate-200"
                  >
                    <Import size={10} />
                  </button>
                  <button
                    ...
                    <EditableText
                      ...
                      onChange={(v) => {
                        ...
                        if (cat === "language") {
                          ...
                          newSkills[sIdx] = { ...newSkills[sIdx], name: nameOnly };
                        } else {
                          newSkills[sIdx] = { ...newSkills[sIdx], name: v };
                        }
                        updateSection("skills", newSkills);
                      }}
                      className="text-[9px] text-slate-700 block"
                    />
                    {isEditing && (
                      <button
                        onClick={() =>
                        ...
                        className="absolute -top-1 -right-1 opacity-0 group-hover/skill:opacity-100 bg-red-500 shadow-sm rounded-full p-0.5 text-white hover:bg-red-600 transition-all scale-75 hover:scale-100"
                      >
                        <X size={7} />
                      </button>
                    )}
      ...
      <div className="w-[70%] p-6 space-y-4">
        {/* Summary */}
        <div>
          <h2 className={mainSectionTitle} style={{ borderColor: accentColor }}>
        ...
        {workExperience && (
          <div>
            <div className="flex justify-between items-end mb-2">
              <h2 className={cn(mainSectionTitle, "flex-1 mb-0")} style={{ borderColor: accentColor }}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {(certificates || []).map((cert, idx) => (
                <div key={cert.id} className="relative group/cert flex justify-between items-start gap-3">
                  {isEditing && (
                    <button
                      onClick={() =>
                        updateSection(
                  ...
                  <div className="flex-1">
                    <EditableText
                      ...
                      className="font-bold text-slate-900 text-[11px] block"
                    ...
                    <EditableText
                      ...
                      className="text-slate-600 text-[10px] block"
                      placeholder="Issuer"
                    />
                    {(isEditing || cert.url) && (
                      <EditableText
