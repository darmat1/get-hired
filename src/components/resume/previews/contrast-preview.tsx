...
export function ContrastPreview({ data, onChange, isEditing }: Props) {
  ...
  // Default values
  const sidebarColor = customization?.sidebarColor || "#1e1b4b";
  ...
  return (
    <div className="flex h-full min-h-[800px] w-full bg-white shadow-sm overflow-hidden font-sans relative group/preview">
      {/* MAIN CONTENT (Left ~65%, plain) */}
      <div className="w-[65%] p-8 pt-12 flex flex-col">
        <div className="mb-8">
          <div
            className="text-3xl font-bold uppercase tracking-wide leading-tight mb-2 flex flex-col gap-1"
                                ...
                                {isEditing ? (
                                  <div className="flex items-center gap-1">
                                    ...
                                    <select
                                      ...
                                      className="bg-transparent border-none p-0 m-0 outline-none text-[10px] text-slate-500 hover:text-blue-600 transition-colors cursor-pointer appearance-none"
                                    >
                                      <option value="">
                                        {getTranslation(
              ...
              {(education || []).map((edu, idx) => (
                <div key={edu.id} className="relative group/edu pb-1">
                  ...
                  <EditableText
                    ...
                    className="text-[12px] text-slate-600 block"
          ...
          <div className="mt-2">
            ...
            <div className="space-y-4">
              {(certificates || []).map((cert, idx) => (
                <div
                  key={cert.id}
                  className="relative group/cert flex justify-between items-start gap-2 pb-1"
                >
                  {isEditing && (
                    <button
                      onClick={() => removeCertificate(cert.id)}
                      className="absolute -right-2 -top-2 opacity-0 group-hover/cert:opacity-100 p-1.5 bg-white shadow-md border border-slate-200 rounded-md text-red-400 hover:text-red-600 hover:bg-red-50 transition-all"
                  ...
                  <EditableText
                    ...
                    className="text-[10px] text-slate-500 shrink-0 text-right mt-1"
                    placeholder="Date"
                  />
                </div>
              ))}
