    ...
    <div className="bg-card text-card-foreground rounded-lg border border-border shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {templates.map(template => (
          <div
            key={template.id}
            className={`border-2 rounded-lg p-4 cursor-pointer transition-all duration-200 ${selectedTemplate === template.id
          ...
          >
            <div className="aspect-[3/4] bg-muted rounded-md mb-3 flex items-center justify-center border border-border/50">
              <span className="text-muted-foreground text-sm">{t('template.preview')}</span>
            </div>
            ...
            {selectedTemplate === template.id && (
              <div className="mt-2 flex items-center gap-1">
                <span className="text-primary text-sm font-medium">✓ {t('template.selected')}</span>
              </div>
            )}
          </div>
        ))}
