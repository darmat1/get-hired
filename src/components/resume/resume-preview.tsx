...
export function ResumePreview({ data }: ResumePreviewProps) {
  ...
  return (
    <div className="bg-card text-card-foreground rounded-lg border border-border shadow-md p-6 overflow-hidden">
      <h3 className="text-lg font-semibold mb-6 text-foreground/80">{t('preview.title')}</h3>
        ...
        {personalInfo ? (
          <div className="space-y-6">
            ...
            {personalInfo.summary && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-2">{t('form.summary')}</h2>
            ...
            {workExperience && workExperience.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-3">{t('preview.work_experience')}</h2>
                    ...
                    <div key={exp.id} className="border-l-2 border-gray-200 pl-4">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <div className="text-right text-sm text-gray-500">
                          ...
                          <div>
                            {exp.startDate} - {exp.current ? t('work.current_position') : exp.endDate}
                          </div>
                        </div>
                      </div>
            ...
            {education && education.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-3">{t('preview.education')}</h2>
                <div className="space-y-3">
                  {education.map((edu) => (
                    <div key={edu.id} className="border-l-2 border-gray-200 pl-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right text-sm text-gray-500">
                          <div>
                            {edu.startDate} - {edu.current ? t('work.current_position') : edu.endDate}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {skills && skills.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-3">{t('preview.skills')}</h2>
