        ...
        <View style={styles.body}>
          <View style={styles.sidebar}>
            {technicalAndSoftSkills.length > 0 && (
              <View>
                <Text style={[styles.sidebarTitle, { marginTop: 0 }]}>
                  {getTranslation("form.skills", lang)}
                </Text>
                {technicalAndSoftSkills.map((skill, index) => (
                  <FormattedText
                    key={index}
                    html={`• ${skill.name}`}
              ...
              <View>
                ...
                {certificates.map((cert, index) => (
                  <View key={index} style={{ marginBottom: 8 }}>
                    <FormattedText html={cert.name} style={styles.sidebarItem} />
                    <Text style={styles.sidebarSubItem}>
                      {cert.issuer} · {formatResumeDate(cert.date)}
                    </Text>
                    {cert.url && (
                      <Text style={[styles.sidebarSubItem, { color: accentColor }]}>
                        {cert.url}
                      </Text>
                    )}
                  </View>
