...
export function AIKeyWarning() {
  ...
  const fetchKeys = async () => {
    ...
    try {
      ...
      const hasPersonalKeys =
        keysData.connectedProviders && keysData.connectedProviders.length > 0;
  ...
  const exhaustedDesc =
    t("ai_warning.quota_exhausted_desc") ||
    "Your balance will automatically refill in {timer}. Need more right now? Connect your personal Groq key in 1 minute — it's absolutely free!";
  ...
  return (
    <div className="relative z-[60] animate-in slide-in-from-top-full duration-300">
      <div className="bg-amber-600 border-amber-700 text-white shadow-lg px-4 py-1 border-b transition-colors duration-500">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold leading-tight mb-0.5">
                {quotaExhausted
                  ? t("ai_warning.quota_exhausted_title") ||
                    "⏳ AI Quota Exhausted for Today!"
                  : t("ai_warning.available_title") ||
                    "AI Configuration Required"}
              </p>
              <p className="text-xs opacity-95 leading-snug max-w-2xl">
                {quotaExhausted
                  ? formattedDesc
                  : t("ai_warning.description") ||
                    "To use AI features without daily limits, please connect at least one AI provider."}
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 shrink-0 items-center">
            <Link
              ...
              className="w-full sm:w-auto text-xs px-4 py-1 rounded-md font-bold transition-transform hover:scale-105 bg-white text-amber-600 shadow-md"
            >
              {quotaExhausted
            ...
            <a
              ...
              className="w-full sm:w-auto text-xs px-4 py-1 rounded-md font-medium transition-colors border border-white/40 text-white hover:bg-white/10"
            >
